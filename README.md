...
```
- `isReadingsValid`  
  Flag produced by the firmware indicating whether the sensor reading is considered valid.   (Currently logged but not used by the backend)

- `timestamp_millisec` : Timestamp (in milliseconds since device boot) generated by the ESP32 device
...
- `detected at` : Timestamp (in milliseconds since device boot) generated by the ESP32 device
- `received at` : Timestamp (in seconds (UTC)) recorded by the backend when the MQTT message is received
